...
import { getTrendingRestaurants } from "@/lib/restaurants";
...
export default function TrendingSection() {
  const [items, setItems] = useState<TrendingRestaurant[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    getTrendingRestaurants(10)
      .then(setItems)
      .catch(() => setItems([]))
      ...
      <div className="overflow-x-auto overflow-y-hidden pb-2 -mx-1 scroll-smooth-x">
        <div className="flex gap-4 min-w-0">
          {loading ? (
            ...
            items.map((r) => <TrendingCard key={r._id} restaurant={r} />)
          )}
        </div>
      </div>
    </section>
